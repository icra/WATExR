...
def get_seasonal_forecast(year, season):
    ...
    Returns:
        Int. Subprocess return code. Should be 0 if R script is successful. Datasets
        are saved to 
    ...
    # Process seasoanl data using R. Takes a while!
    print('    Downloading and bias-correcting seasonal forecast data (this usually take around 5 minutes)...')
...
def aggregate_seasonal_forecast(season, year, par_list=['tas', 'pr', 'wind'],
    ...
    """   
    # Check data from 15-member ensemble has been downloaded for specified year/season
    if season == 'winter':
        st_dt = f'{year - 1}1101'
        end_dt = f'{year}0131'
    elif season == 'spring':
    ...
    search_path = os.path.join(base_path,
                               f'Morsa_NIVA_System4_seasonal_15_seasonal_member*_day_{st_dt}-{end_dt}')

    s4_dirs = sorted(glob.glob(search_path))
    
    if len(s4_dirs) != 15:
        message = ('Cached climate data not found for selected year and season.\n'
    ...
    for dir_path in s4_dirs:
        # Get member
        member = os.path.split(dir_path)[1].split('_')[6][-2:]

        fpath = os.path.join(dir_path, 'meteo_file.dat')
