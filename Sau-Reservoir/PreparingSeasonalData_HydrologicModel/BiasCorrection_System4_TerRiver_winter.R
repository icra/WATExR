...
start_date <- match(as.Date(data_model_sub[[1]]$Dates$start[1]), as.Date(data_obs_sub[[1]]$Dates$start))
end_date <- match(as.Date(data_model_sub[[1]]$Dates$start[length(data_model_sub[[1]]$Dates$start)]), 
                  as.Date(data_obs_sub[[1]]$Dates$start))
for (x in 1:length(data_obs_sub)){
  data_obs_sub[[x]]$Data <- data_obs_sub[[x]]$Data[start_date:end_date,,]
  data_obs_sub[[x]]$Dates$start <- data_obs_sub[[x]]$Dates$start[start_date:end_date]
  data_obs_sub[[x]]$Dates$end <- data_obs_sub[[x]]$Dates$end[start_date:end_date]
}
attr(data_obs_sub$pr$Data, "dimensions") <- c("time", "lat", "lon")
attr(data_obs_sub$tas$Data, "dimensions") <- c("time", "lat", "lon")
attr(data_obs_sub$pet$Data, "dimensions") <- c("time", "lat", "lon")
