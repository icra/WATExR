...
#' @import airGR
#' @export
run_gr6j <- function(time, pet, pre, airt = NULL, warmup_unit = NULL, warmup_n = NULL, param, catch_size, out_file, calc_T = FALSE, vector = TRUE, model = NULL){
  
  inputs <- CreateInputsModel(RunModel_GR6J, DatesR = time, Precip = pre, PotEvap = pet)
  if(is.null(warmup_unit) | is.null(warmup_n)){
    opts <- CreateRunOptions(RunModel_GR6J, InputsModel = inputs,IndPeriod_Run = 1:length(time),IndPeriod_WarmUp = NULL)
  }else{
    start = time[1]
    ...
    opts <- CreateRunOptions(RunModel_GR6J, InputsModel = inputs,IndPeriod_Run = run_ind, IndPeriod_WarmUp = warmup_ind)
  }
  
  mod1 = RunModel_GR6J(InputsModel = inputs, RunOptions = opts, Param = param)
  message('GR6J simulation compete!')
  ...
  if(calc_T){
    ...
    if(is.null(warmup_unit) | is.null(warmup_n)){
      airt = airt #Stefan, H.G. and E.B. Preudíhomme. 1993. Stream temperature estimation from air temperature. Water Resources Bulletin 29(1): 27-45
    ...
    flow$T_degC<- inflow.temp
  }
  qdf$flow <- (qdf[,2] * catch_size *1000000)/(1000*60*60*24)
  
  
  
  # flow$year <- year(flow[,1])
  
  #flow <- flow[-which(flow$year==unique(flow$year)[1]),]
