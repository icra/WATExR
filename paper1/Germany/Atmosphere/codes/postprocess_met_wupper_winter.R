...
rea <-  get(load("C:/Users/shikhani/Documents/LINUX/ERA5/ERA5_daily_Interpolated_March2020.RData"))
obs <-  get(load("C:/Users/shikhani/Documents/LINUX/ERA5/ERA5_daily_Interpolated_March2020.RData"))
hind <- get(load("C:\\Users\\shikhani\\Documents\\LINUX/System_5/Data_final_2019/System5_final_11_12_1_2_uas_vas_tdps_tas_cc_tp_rsds_rlds_petH_BCcross.rda"))
obs$sp <- NULL
rea$sp <- NULL
names(rea) <- names(hind)
...
# Compute wss
wss <- obs$uas
wss$Data <- sqrt(obs$uas$Data^2 + obs$vas$Data^2)
...
wss <- rea$uas
wss$Data <- sqrt(rea$uas$Data^2 + rea$vas$Data^2)
# Define correctly the metadata of the object:
wss$Variable$varName <- "wss"
attr(wss$Variable,"units") <- "m s**-1"
attr(wss$Variable,"description") <- "Near-Surface Wind Speed"
attr(wss$Variable,"longname") <- "wss"
rea <- c(rea, "wss" = list(wss))


wss <- hind$uas
wss$Data <- sqrt(hind$uas$Data^2 + hind$vas$Data^2)
# Define correctly the metadata of the object:
wss$Variable$varName <- "wss"
attr(wss$Variable,"units") <- "m s**-1"
attr(wss$Variable,"description") <- "Near-Surface Wind Speed"
attr(wss$Variable,"longname") <- "wss"
hind <- c(hind, "wss" = list(wss))
...
rea.wu <- lapply(1:length(rea), function(i){
  ...
  if (!identical(getSeason(hind$uas), c(11, 12, 1, 2))) {
...
#output.dir.met <- "Documents/System_5/Data_for_models/1year_warmup/Autumn/"
output.dir.met <- "C:/Users/shikhani/Documents/LINUX/System_5/Data_for_models/2year_warmup/winternewnew"
...
#obs <- get(load("C:\\Users\\shikhani\\Documents\\met_era5/ERA5_daily_Interpolated_NewRad.RData"))
